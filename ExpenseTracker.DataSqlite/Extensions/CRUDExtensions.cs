...
namespace ExpenseTracker.DataSqlite.Extensions
{
    // install Dapper
    internal static class CRUDExtensions
    {
        ...
        public static T InsertScalar<T>(this IDbConnection cnn, string tableName, string tableColumns, string values, object param, IDbTransaction transaction = null)
        {
            ...
            query.Append(") VALUES (");
            query.Append(values);
            query.Append("); SELECT last_insert_rowid() id");
        ...
        {
            //var query = "SELECT "+ tableColumns+" FROM " + tableName+" LIMIT 50";
            StringBuilder query = new StringBuilder();
            query.Append("SELECT ");
            query.Append(tableColumns);
            query.Append(" FROM ");
            query.Append(tableName);
            query.Append(" order by created_in desc LIMIT ");
            query.Append(topNo);
            return cnn.Query<T>(query.ToString(), transaction: transaction, commandType: CommandType.Text);
        ...
        public static int ExecuteQuery(this IDbConnection cnn, string query, IDbTransaction transaction = null)
        {
            return cnn.Execute(query, null, transaction: transaction);
        }
